...
function TurismoPage() {
  ...
  const [lugaresTurismo, setLugaresTurismo] = useState([]);
  ...
  useEffect(() => {
    if (lugares && lugares.length > 0) {
      // Filtrar lugares que pertenecen a la categoría "Turismo"
      const filtrados = lugares.filter(lugar => {
        const categoriaNombre = lugar.categoria?.nombre || '';
        return categoriaNombre.toLowerCase().includes('turismo');
      });
      
      setLugaresTurismo(filtrados);
  ...
  if (loading) {
    return (
      <div className="loader-container">
        ...
        <p className="loader-text">Cargando turismo...</p>
      </div>
    );
  }
  ...
  return (
    <main className="main-container">
      <CategoryFilters currentCategory="turismo" />

      <section id="turismo" className="section-group">
        <h2 className="section-group-title">Turismo</h2>
        ...
        {hayAgrupacion ? (
          ...
          <div className="section-group-container">
            <div className="cards-container">
              {ordenarPorNivel([...lugaresTurismo]).map(lugar => (
                <CardEmpresa key={lugar._id} item={lugar} />
              ))}
            </div>
          </div>
